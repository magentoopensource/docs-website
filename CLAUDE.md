...
### Testing
**Note:** This application currently does not have a test suite configured. PHPUnit is listed as a dev dependency but no test files or phpunit.xml configuration exists yet.
...
- Main content in `resources/docs/main/` (synchronized via `bash bin/checkout_latest_docs.sh`)
...
**Search**: Algolia integration (requires ALGOLIA_ID and ALGOLIA_SEARCH_KEY in .env)
**Asset Pipeline**: Vite for CSS/JS compilation with Laravel Vite plugin
**External Content**: Documentation content is managed separately and synced via scripts, not stored in main repository
...
**Database**: No database required - this is a static documentation site with file-based content
**Routing**: Simple route structure in `routes/web.php` - docs routes, category pages, and team page
**Asset Compilation**: Uses Vite with `resources/css/app.css` and `resources/js/app.js` as entry points
**Caching Strategy**: Documentation uses Laravel's cache system (5-minute page cache, 1-hour index cache)
**Version Management**: Supported versions are hardcoded in `Documentation::getDocVersions()`
